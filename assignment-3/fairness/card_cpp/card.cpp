...
void mimc_hash_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void mimc_hash_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
void card_5_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void card_5_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 49763;}

uint get_number_of_components() {return 141;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 28412;}
...
ctx->componentMemory[coffset].templateName = "mimc_hash";
ctx->componentMemory[coffset].signalStart = soffset;
...
ctx->componentMemory[coffset].templateName = "card";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 5;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[42];
}
...
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "calcHash";
mySubcomponents[aux_create+i] = aux_cmp_num;
mimc_hash_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
...
uint csoffset = mySignalStart+47989;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "salting";
mySubcomponents[aux_create+i] = aux_cmp_num;
mimc_hash_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
...
uint csoffset = mySignalStart+47905;
for (uint i = 0; i < 1; i++) {
...
uint csoffset = mySignalStart+24856;
uint aux_dimensions[1] = {13};
for (uint i = 0; i < 13; i++) {
std::string new_cmp_name = "check_suite_old"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
mimc_hash_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
...
uint csoffset = mySignalStart+1807;
uint aux_dimensions[1] = {13};
for (uint i = 0; i < 13; i++) {
std::string new_cmp_name = "check_suite_new"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
mimc_hash_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
...
uint csoffset = mySignalStart+47911;
uint aux_dimensions[1] = {13};
for (uint i = 0; i < 13; i++) {
std::string new_cmp_name = "new_equal"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
IsEqual_4_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 6 ;
aux_cmp_num += 2;
...
// need to run sub component
assert(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
mimc_hash_2_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
// load src
Fr_sub(&expaux[0],&circuitConstants[3],&ctx->signalValues[ctx->componentMemory[mySubcomponents[2]].signalStart + 0]); // line circom 33
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_eq(&expaux[0],&signalValues[mySignalStart + 7],&circuitConstants[3]); // line circom 34
...
Fr_lt(&expaux[0],&lvar[13],&circuitConstants[27]); // line circom 44
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[13])) + 3);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&lvar[((1 * Fr_toInt(&lvar[13])) + 0)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
mimc_hash_2_run(mySubcomponents[cmp_index_ref],ctx);
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[13])) + 8)];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[13])) + 3)]].signalStart + 0]);
}
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[13])) + 16);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[13])) + 8)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
mimc_hash_2_run(mySubcomponents[cmp_index_ref],ctx);
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[13])) + 21)];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[13])) + 16)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[13];
// load src
Fr_add(&expaux[0],&lvar[13],&circuitConstants[3]); // line circom 44
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[13],&circuitConstants[27]); // line circom 44
...
Fr_lt(&expaux[0],&lvar[14],&circuitConstants[27]); // line circom 59
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[14])) + 29);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[14])) + 21)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
IsEqual_4_run(mySubcomponents[cmp_index_ref],ctx);
...
PFrElement aux_dest = &lvar[13];
// load src
Fr_add(&expaux[0],&lvar[13],&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[14])) + 29)]].signalStart + 0]); // line circom 64
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[14];
// load src
Fr_add(&expaux[0],&lvar[14],&circuitConstants[3]); // line circom 59
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[14],&circuitConstants[27]); // line circom 59
}
Fr_eq(&expaux[0],&lvar[13],&circuitConstants[3]); // line circom 66
